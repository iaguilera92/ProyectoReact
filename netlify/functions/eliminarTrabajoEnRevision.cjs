...
const FILE_KEY = "TrabajosEnRevision.xlsx"; // 👈 Cambiado
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const { Id } = body; // 👈 recibimos Id

        if (!Id) {
            return {
                statusCode: 400,
                headers: corsHeaders,
                body: JSON.stringify({ message: "Falta el campo Id" }),
            };
        }
        ...
        const datosFiltrados = datos.filter(row => Number(row.Id) !== Number(Id));

        if (datosFiltrados.length === datos.length) {
            console.warn("⚠️ No se encontró el trabajo Id:", Id);
            return {
                statusCode: 404,
                headers: corsHeaders,
        ...
        console.log("🗑️ Trabajo eliminado Id:", Id);
