          ...
          {parsed && (
            <Box sx={{ mt: 0.75, borderRadius: 1.5, border: "1px solid rgba(239,68,68,0.22)", overflow: "hidden" }}>
              ...
              <Box
                ...
                sx={{ px: 1.5, py: 0.45, display: "flex", alignItems: "center", gap: 0.5, cursor: "pointer", bgcolor: dark ? "rgba(0,0,0,0.2)" : "rgba(0,0,0,0.04)", "&:hover": { bgcolor: dark ? "rgba(0,0,0,0.3)" : "rgba(0,0,0,0.07)" } }}
              >
    ...
    <Box sx={{ flex: 1, overflowY: "auto", bgcolor: C.bg, px: { xs: 1.5, md: 3 }, py: { xs: 2, md: 3 } }}>
          ...
          >
            <Box sx={{
              ...
              <Typography sx={{ fontSize: "0.82rem", fontWeight: 700, color: "#fff", flex: 1, minWidth: 0 }}>
                {ultima.tests_failed} test{ultima.tests_failed !== 1 ? "s" : ""} fallido{ultima.tests_failed !== 1 ? "s" : ""}
                {primerFallo && (
                  <Box component="span" sx={{ fontWeight: 400, opacity: 0.88 }}>
                    {" "}· {primerFallo.nombre}
                  </Box>
                )}
              </Typography>
      ...
      {!loading && ultima && (
        <>
          ...
          >
            <Paper elevation={0} sx={{ mb: 2.5, borderRadius: 2.5, border: `1px solid ${ok ? "rgba(34,197,94,0.28)" : "rgba(239,68,68,0.28)"}`, bgcolor: ok ? "rgba(34,197,94,0.05)" : "rgba(239,68,68,0.05)", overflow: "hidden" }}>
              <LinearProgress variant="determinate" value={porcentaje} sx={{ height: 3, bgcolor: "transparent", "& .MuiLinearProgress-bar": { bgcolor: ok ? "#4ade80" : "#f87171", transition: "none" } }} />
              <Box sx={{ px: { xs: 2, md: 2.5 }, py: 2, display: "flex", flexWrap: "wrap", gap: 2.5, alignItems: "center" }}>
                <Box sx={{ flexShrink: 0, fontSize: "2.2rem", lineHeight: 1 }}>{ok ? "✅" : "❌"}</Box>

                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap", mb: 0.4 }}>
                    <Typography sx={{ fontSize: "1.05rem", fontWeight: 700, color: C.text }}>
                      {ok ? "Todos los tests pasaron" : "Hay tests fallidos"}
                    </Typography>
                    <EstadoBadge estado={ultima.estado} />
                  </Box>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, flexWrap: "wrap" }}>
                    <AccessTimeIcon sx={{ fontSize: 13, color: C.textDim }} />
                    <Typography sx={{ fontSize: "0.78rem", color: C.textDim }}>{formatFecha(ultima.created_at)}</Typography>
                    <Typography sx={{ fontSize: "0.78rem", color: C.textFaint }}>· Build #{ultima.pipeline_id}</Typography>
                    <Tooltip title="Ver en Azure DevOps" arrow>
                      <IconButton
                        size="small"
                        component="a"
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        sx={{ p: 0.15, color: C.textFaint, "&:hover": { color: "#60a5fa" } }}
                      >
                        <OpenInNewIcon sx={{ fontSize: 13 }} />
                      </IconButton>
                    </Tooltip>
                ...
                <Box sx={{ display: "flex", gap: { xs: 2, md: 2.5 }, flexShrink: 0 }}>
                  {[
                    ...
                    <Box key={m.label} sx={{ textAlign: "center" }}>
                      <Typography sx={{ fontSize: "1.2rem", fontWeight: 800, color: m.color, lineHeight: 1 }}>{m.val}</Typography>
                      <Typography sx={{ fontSize: "0.62rem", color: C.label, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", mt: 0.25 }}>{m.label}</Typography>
                    </Box>
                  ))}
                </Box>
              </Box>
            ...
            <motion.div variants={fadein} initial="hidden" animate="show" transition={{ duration: 0.35, delay: 0.12 }}>
              ...
              >
                <Box sx={{ px: 2, py: 1.5, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <Typography sx={{ fontSize: "0.72rem", color: C.textDim }}>
                    {detalleVisible.filter(t => t.estado === "passed").length}/{detalleVisible.length} pasaron
                  </Typography>
                </Box>
